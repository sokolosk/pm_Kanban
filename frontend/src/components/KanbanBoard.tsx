...
export const KanbanBoard = ({ token, user, onLogout }: KanbanBoardProps) => {
  ...
  const persistBoard = async (updatedBoard: BoardData) => {
    if (!boardId) return;
    setSaving(true);
    try {
      const saved = await saveBoard(token, boardId, updatedBoard);
      setBoard(saved);
    } catch (err) {
